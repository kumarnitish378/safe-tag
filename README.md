...
| 🔲 | **QR code endpoint** — `GET /qr/:tagId` streams a live 300×300 PNG, cached 24h |
| 🎨 | **Liquid UI** — glassmorphism cards, scroll-reveal, animated counters, mobile hamburger nav |
| 🎉 | **Onboarding flow** — guided 3-step welcome page after registration |
| 💥 | **Error pages** — distinct 404 and 500 pages |
...
`db push` creates `prisma/dev.db` and applies the schema. `db seed` populates test users, tags, and products.
...
Seed credentials are defined in `prisma/seed.js`. Run `npx prisma db seed` to populate them locally.

> **Do not share seed credentials publicly. This section is intentionally kept minimal.**

| Role | How to get credentials |
|---|---|
| Admin | See `prisma/seed.js` |
| Customer | See `prisma/seed.js` |
| Manufacturer | See `prisma/seed.js` |

### Test Tags

Test tag IDs and security keys are generated by the seed script. Check `prisma/seed.js` for the exact values used locally.
...
|---|---|
| Environment | Node |
| Build command | `npm ci --include=dev && node scripts/patch-schema-for-prod.js && npx prisma generate && npx tailwindcss -i ./public/css/input.css -o ./public/css/safetag.build.css --minify && npx prisma db push` |
| Start command | `node server.js` |
| Root directory | `/` (project root) |

The build command patches `prisma/schema.prisma` from `sqlite` → `postgresql` before generating the client. Set `DATABASE_URL` to a PostgreSQL connection string in the Render environment dashboard.

> **Note:** `render.yaml` is present but Render uses dashboard settings for manually-created services. Update the build command in the Render dashboard directly.
