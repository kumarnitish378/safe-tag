...
// 9-char base62 single token — replaces separate tagId + securityKey.
// 62^9 = 13.5 quadrillion combinations, covers 3× world population with
// 0.00018% random-guess probability (vs brute-force infeasible).
function generateTagCode() {
  const arr = [];
  for (let i = 0; i < 9; i++) arr.push(BASE62[crypto.randomInt(BASE62.length)]);
  return arr.join('');
}
...
function formatTag(tag, includeProfile = false) {
  const d = {
    tag_id: tag.tagId,
    // Effective type drives dashboard actions; origin_type preserves the
    // manufactured identity ('universal' for buyer-choose tags).
    tag_type: tag.resolvedType || tag.tagType || 'medical',
    origin_type: tag.tagType || 'medical',
    resolved_type: tag.resolvedType || null,
...
function calcBatchPrice(qty) {
  // Returns total charge in paise (INR × 100)
  const rate = qty <= 100 ? 5 : qty <= 1000 ? 3 : 1.5;
...
function formatBatch(b) {
  return {
    ...
    quantity: b.quantity,
    paid_amount: b.paidAmount,           // paise
    payment_status: b.paymentStatus,     // pending | paid | failed
